...
func _ready():
	Events.card_aim_started.connect(_on_card_drag_or_aiming_stated)
	Events.card_drag_started.connect(_on_card_drag_or_aiming_stated)
...
func _on_card_drag_or_aiming_stated(used_card:CardUI):
	if used_card == self:
		return
	disabled = true
